...
impl NodeKind {
    ...
    /// The thing about this node that can surprise someone, or `None` if nothing does.
    ///
    /// Kept here rather than in the editor because it is a fact about the *language*,
    /// not about drawing: the same words belong in the docs, and a second copy would
    /// drift. Only nodes with a genuine sharp edge answer — an icon on every node is an
    /// icon nobody reads.
