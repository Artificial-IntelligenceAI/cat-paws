...
/// Solarized's stock dark body text. Kept for completeness of the palette; the
/// UI uses the brighter BASE1 for legibility at small sizes.
#[allow(dead_code)]
...
impl Palette {
    pub fn new(mode: Mode) -> Palette {
        match mode {
            Mode::Dark => Palette {
                ...
                node_outline: BASE01,
                // One step brighter than stock Solarized dark body text. BASE0 on
                // BASE03 is handsome in a terminal but too weak for small labels
                // sitting on coloured node bodies.
                text: BASE1,
                text_strong: BASE2,
                text_faint: BASE00,
                grid: rgb(0x00343f),
            ...
            Mode::Light => Palette {
                ...
                text: BASE01,
                text_strong: BASE03,
                // BASE1 was so pale on BASE3 it was effectively invisible.
                text_faint: BASE00,
                grid: rgb(0xf7f1de),
                grid_strong: BASE2,
                exec_wire: BASE01,
    ...
    ///
    /// This also sets the text sizes. egui's defaults are tuned for dense tool
    /// UIs and read as small here, so every style is bumped a couple of points.
    pub fn apply(&self, ctx: &egui::Context) {
        ...
        // Written into both the light and dark style slots. Cat Paws has its own
        // theme toggle, so the OS setting must not get a vote.
